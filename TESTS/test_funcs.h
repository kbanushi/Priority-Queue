...
_PQ * _pq_create(int capacity, int min_heap) {
  ...
  int i;
  _PQ * q = (_PQ *)malloc(sizeof(_PQ));

  q->data = (_ENTRY *)malloc(capacity * sizeof(_ENTRY));
  q->cap = capacity;
 ...
 */
int empty_and_check(pqueue *q, _PQ *_q) {
  int id, _id;
  double p, _p;
  int bad;
  int passed = 1;

  if(q->size() != _pq_size(_q) ) {
 ...
 */
void change_priorities(pqueue *q, _PQ *_q) {

  int n=q->capacity();
  int id;

  if(_pq_size(_q) != q->size()){
...
void change_priorities_rt(pqueue *q) {
...
// (unimplemented)
//
int scan_get_priorities(pqueue *q, _PQ *_q) {
  double p, _p;
 ...
 *  (unimplemented)
 */
int empty_via_rem_by_id(pqueue *q, _PQ *_q) {
  int n=q->capacity();
  double p, _p;
  int found;
  int id;
...
int one_del_top(pqueue *q, _PQ *_q) {
...
//
// expects identical results.  reports success/failure
int one_insert(pqueue *q, _PQ *_q, int id, double p) {

  if(q->insert( id, p) != _pq_insert(_q, id, p)){
...
//
// expects identical results.  reports success/failure
int one_remove_by_id(pqueue *q, _PQ *_q, int id) {

  if(q->remove_by_id( id) != _pq_remove_by_id(_q, id)){
    return 0;
  }
  return 1;
}
...
//
// expects identical results.  reports success/failure
int one_change_priority(pqueue *q, _PQ *_q, int id, double newp) {
  int success, _success;

  success = q->change_priority( id, newp);
...
//
// expects identical results.  reports success/failure
int one_get_priority(pqueue *q, _PQ *_q, int id) {
  int success, _success;
  double p, _p;

  success = q->get_priority( id, p);
...
void gen_pairs(int n, int **p_ids, double **p_priorities) {
  ...
  double *priorities = (double *)malloc(n*sizeof(double));
  int i, j, tmp;
...
void init_qs(int ids[], double p[], int n, int cap, 
                int minAtTop, pqueue **q, _PQ **_q) {

  int i;
  *q = new pqueue(cap, minAtTop);
